...
subsetclust1 <- mlbdata[, c("X23.payroll", "X22.payroll", "X21.payroll", "X20.payroll", "City.Population", "media.market.size", 
                            "Team.valuation", "X23.attendence.g","X22.attendence.g","X21.attendence.g")]
...
fviz_nbclust(
  ...
  kmeans,
  method = "wss",
  k.max = 25, #Max number of clusters on graph
  verbose = FALSE
)
...
kmeans.re <- kmeans(subsetclust1, centers = 3, nstart = 20) #3 centroids, 20 iterations
kmeans.re 
